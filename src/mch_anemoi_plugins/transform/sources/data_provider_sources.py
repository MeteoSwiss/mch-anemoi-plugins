...
def get_fieldlist_from_data_provider(
    ...
    **retriever_kwargs: Any,
    ...
    interp_spatial = expanded_kwargs.pop("interp_spatial", None)
    data = provider.provide(source, param, dates, **expanded_kwargs).drop_duplicates(...)
    time_dim = ("forecast_reference_time"
                if "forecast_reference_time" in data.dims else "time" if "time" in data.dims else None)
    if time_dim is None:
        raise ValueError(
    ...
    if resample is not None:
        ...
        data_next = provider.provide(source, param,
                                     [d + pd.to_timedelta("1d") for d in dates], **expanded_kwargs).drop_duplicates(
                                         ...)  # otherwise we don't get reampled data at the end of the period
        data_next = check_indexing(
            data_next, time_dim="forecast_reference_time" if "forecast_reference_time" in data_next.dims else "time")
    ...
    if time_dim is not None:  # e.g. not a forcing dataset
        data = data.sortby(time_dim).sel({time_dim: dates},
                                         method="nearest")  # return only the relevant time for daily arrays
        data[time_dim] = dates
    else:
        data = data.assign_coords(time=dates)  # if no time dimension, assign the dates as a coordinate
        time_dim = "time"
...
def make_source_class(source_name: str):
    ...
    def __init__(self, context, param, **retriever_kwargs):
        super(self.__class__, self).__init__(context, source_name, param, **retriever_kwargs)
